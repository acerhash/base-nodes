...
export default function App() {
  ...
  // Peer Connection Health Chart Auto Refresh & Breathe Animation State
  const [isChartAutoRefreshing, setIsChartAutoRefreshing] = useState(true);
  const [isChartRefreshing, setIsChartRefreshing] = useState(false);
  const [lastChartRefreshedAt, setLastChartRefreshedAt] = useState(() =>
    new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })
  );

  // Auto-refresh interval effect: triggers chart update & breathe animation on trend arrows
  useEffect(() => {
    if (!isChartAutoRefreshing) return;
    const interval = setInterval(() => {
      setIsChartRefreshing(true);
      setLastChartRefreshedAt(
        new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })
      );
    ...
    return () => clearInterval(interval);
  }, [isChartAutoRefreshing]);

  const triggerManualChartRefresh = () => {
    setIsChartRefreshing(true);
    setLastChartRefreshedAt(
      new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })
    );
                      ...
                      <div ref={chartContainerRef} className="p-4 bg-slate-50/90 rounded-2xl border border-slate-200 space-y-3">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-2">
                          ...
                          <div className="flex items-center gap-2 text-[11px] font-mono flex-wrap">
                                  ...
                                  >
                                    ...
                                    <span className="text-slate-800 font-bold flex items-center gap-1">
                                      ...
                                      {peerDiff15m > 0 ? (
                                        ...
                                        <ArrowDownRight className={`w-3.5 h-3.5 text-rose-600 stroke-[2.5] transition-all duration-300 ${isChartRefreshing ? 'animate-breathe' : ''}`} />
                                      ) : (
                                        <Minus className={`w-3.5 h-3.5 text-slate-500 stroke-[2.5] transition-all duration-300 ${isChartRefreshing ? 'animate-breathe' : ''}`} />
                                      )}
                                    </span>
                                    {peerDiff15m > 0 ? (
                            ...
                            <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-slate-300 shadow-xs font-mono">
                              <button
                                onClick={() => setIsChartAutoRefreshing((prev) => !prev)}
                                className={`px-2 py-0.5 text-[10px] font-extrabold rounded-lg transition-all cursor-pointer flex items-center gap-1.5 ${
                                  isChartAutoRefreshing
                                    ? 'bg-emerald-50 text-emerald-800 border border-emerald-300/80'
                                    : 'bg-slate-100 text-slate-600 border border-slate-200'
                                }`}
                                title={isChartAutoRefreshing ? "Auto-refresh active (every 4s). Click to pause." : "Auto-refresh paused. Click to enable."}
                              >
                                <span className={`w-2 h-2 rounded-full ${isChartAutoRefreshing ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`} />
                                <span>Auto: {isChartAutoRefreshing ? 'ON' : 'OFF'}</span>
                              </button>

                              <button
                                onClick={triggerManualChartRefresh}
                                className="px-2 py-0.5 bg-slate-50 hover:bg-slate-100 text-slate-700 hover:text-blue-600 border border-slate-200 hover:border-blue-300 font-bold text-[10px] rounded-lg transition-all flex items-center gap-1 shrink-0 cursor-pointer shadow-2xs font-mono"
                                title={`Click to manually refresh chart data and trigger trend breathe effect (Last updated at ${lastChartRefreshedAt})`}
                              >
                                <RefreshCw className={`w-3 h-3 text-blue-600 ${isChartRefreshing ? 'animate-spin' : ''}`} />
                                <span>Refresh</span>
                              </button>
                            </div>
